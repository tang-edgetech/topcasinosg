...
// The section itself is transparent by default (its own bgType/bgFrom/bgTo
// bleed background, if the admin sets one, is independent of the inner
// gold-gradient card below, which stays a fixed decorative element rather
// than admin-authored background data).
export default async function RegionExplorerSection({ section }: { section: PageSection }) {
  ...
  return (
    <section
      ...
      className={sectionClassName(hasBleedBg ? "section--region-explorer section--bg" : "section--region-explorer", section)}
      style={bgStyle as CSSProperties}
    >
      <div className="section-container relative z-10 flex flex-col items-center gap-6 rounded-xl bg-gradient-to-r from-secondary-600 to-secondary-500 py-12">
