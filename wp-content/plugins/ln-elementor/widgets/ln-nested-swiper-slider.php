...
class Nested_Swiper_Slider extends Widget_Nested_Base {
	...
	public function get_name() {
		return 'nested-swiper-slider';
	}

	public function get_title() {
		return esc_html__( 'Swiper Slider', 'ln-elementor' );
	}
	...
	public function get_keywords() {
		return [ 'nested', 'carousel', 'swiper', 'slider', 'ln' ];
	}
	...
	protected function get_default_children_title() {
		/* translators: %d: Tab index. */
		return esc_html__( 'Tab #%d', 'ln-elementor' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'section_tabs', [
			'label' => esc_html__( 'Tabs', 'ln-elementor' ),
		] );
		...
		$repeater->add_control( 'tab_title', [
			...
			'default' => esc_html__( 'Tab Title', 'ln-elementor' ),
			'placeholder' => esc_html__( 'Tab Title', 'ln-elementor' ),
			'label_block' => true,
			'dynamic' => [
				'active' => true,
			],
		...
		$repeater->add_control(
			'tab_icon_active',
			[
				'label' => esc_html__( 'Active Icon', 'ln-elementor' ),
				'type' => Controls_Manager::ICONS,
				'fa4compatibility' => 'icon',
				'skin' => 'inline',
				'label_block' => false,
		...
		$repeater->add_control(
			'element_id',
			[
				'label' => esc_html__( 'CSS ID', 'ln-elementor' ),
				'type' => Controls_Manager::TEXT,
				'default' => '',
				'ai' => [
					'active' => false,
				...
				'title' => esc_html__( 'Add your custom id WITHOUT the Pound key. e.g: my-id', 'ln-elementor' ),
		...
		$this->add_control( 'tabs', [
			'label' => esc_html__( 'Tabs Items', 'ln-elementor' ),
			'type' => Control_Nested_Repeater::CONTROL_TYPE,
			'fields' => $repeater->get_controls(),
			'default' => [
				[
					'tab_title' => esc_html__( 'Tab #1', 'ln-elementor' ),
				],
				[
					'tab_title' => esc_html__( 'Tab #2', 'ln-elementor' ),
				],
				[
					'tab_title' => esc_html__( 'Tab #3', 'ln-elementor' ),
				],
			],
			'title_field' => '{{{ tab_title }}}',
			'button_text' => esc_html__( 'Add Tab', 'ln-elementor' ),
		] );
		...
		$this->add_responsive_control( 'tabs_direction', [
			'label' => esc_html__( 'Direction', 'ln-elementor' ),
			'type' => Controls_Manager::CHOOSE,
			'options' => [
				'block-start' => [
					'title' => esc_html__( 'Above', 'ln-elementor' ),
					'icon' => 'eicon-v-align-top',
				],
				'block-end' => [
					'title' => esc_html__( 'Below', 'ln-elementor' ),
					'icon' => 'eicon-v-align-bottom',
				],
				'inline-end' => [
					'title' => esc_html__( 'After', 'ln-elementor' ),
					'icon' => 'eicon-h-align-' . $end,
				],
				'inline-start' => [
					'title' => esc_html__( 'Before', 'ln-elementor' ),
		...
		$this->add_responsive_control( 'tabs_justify_horizontal', [
			'label' => esc_html__( 'Justify', 'ln-elementor' ),
			'type' => Controls_Manager::CHOOSE,
			'options' => [
				'start' => [
				...
				'stretch' => [
					'title' => esc_html__( 'Stretch', 'ln-elementor' ),
		...
		$this->add_responsive_control( 'title_alignment', [
			'label' => esc_html__( 'Align Title', 'ln-elementor' ),
			'type' => Controls_Manager::CHOOSE,
			'options' => [
				'start' => [
					'title' => esc_html__( 'Start', 'ln-elementor' ),
		...
		$this->start_controls_section( 'section_tabs_responsive', [
			'label' => esc_html__( 'Additional Settings', 'ln-elementor' ),
		] );

		$this->add_responsive_control(
			'horizontal_scroll',
			[
				'label' => esc_html__( 'Horizontal Scroll', 'ln-elementor' ),
				'type' => Controls_Manager::SELECT,
				'description' => esc_html__( 'Note: Scroll tabs if they don’t fit into their parent container.', 'ln-elementor' ),
				'options' => [
					'disable' => esc_html__( 'Disable', 'ln-elementor' ),
					'enable' => esc_html__( 'Enable', 'ln-elementor' ),
				],
				'default' => 'disable',
				'selectors_dictionary' => [
		...
		$dropdown_options = [
			'none' => esc_html__( 'None', 'ln-elementor' ),
		];
		...
		foreach ( Plugin::$instance->breakpoints->get_active_breakpoints() as $breakpoint_key => $breakpoint_instance ) {
			...
			$dropdown_options[ $breakpoint_key ] = sprintf(
				...
				esc_html__( '%1$s (%2$s %3$dpx)', 'ln-elementor' ),
		...
		$this->add_control(
			'breakpoint_selector',
			[
				'label' => esc_html__( 'Breakpoint', 'ln-elementor' ),
				'type' => Controls_Manager::SELECT,
				'description' => esc_html__( 'Note: Choose at which breakpoint tabs will automatically switch to a vertical (“accordion”) layout.', 'ln-elementor' ),
		...
		$this->add_responsive_control( 'tabs_title_space_between', [
			'label' => esc_html__( 'Gap between tabs', 'ln-elementor' ),
			'type' => Controls_Manager::SLIDER,
		...
		$this->add_responsive_control( 'tabs_title_spacing', [
			'label' => esc_html__( 'Distance from content', 'ln-elementor' ),
			'type' => Controls_Manager::SLIDER,
			'size_units' => [ 'px', 'em', 'rem', 'custom' ],
			'range' => [
				'px' => [
		...
		$this->add_group_control(
			...
			[
				...
				'fields_options' => [
					...
					'width' => [
						'label' => esc_html__( 'Border Width', 'ln-elementor' ),
					],
				],
			]
		);
		...
		$this->start_controls_tab(
			'tabs_title_hover',
			[
				'label' => esc_html__( 'Hover', 'ln-elementor' ),
			]
		);
		...
		$this->add_control(
			'hover_animation',
			[
				'label' => esc_html__( 'Hover Animation', 'ln-elementor' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Transition Duration', 'ln-elementor' ) . ' (s)',
				'type' => Controls_Manager::SLIDER,
				'selectors' => [
		...
		$this->start_controls_tab(
			'tabs_title_active',
			[
				'label' => esc_html__( 'Active', 'ln-elementor' ),
			]
		);
		...
		$this->start_controls_section( 'section_title_style', [
			'label' => esc_html__( 'Titles', 'ln-elementor' ),
			'tab' => Controls_Manager::TAB_STYLE,
		] );
		...
		$this->add_responsive_control( 'icon_position', [
			'label' => esc_html__( 'Position', 'ln-elementor' ),
			'type' => Controls_Manager::CHOOSE,
			'options' => [
				'block-start' => [
					'title' => esc_html__( 'Above', 'ln-elementor' ),
	...
	protected function render() {
		...
		$this->add_render_attribute( 'elementor-tabs', [
			...
			'aria-label' => esc_html__( 'Tabs. Open items with Enter or Space, close with Escape and navigate using the Arrow keys.', 'ln-elementor' ),
		] );
	...
	protected function content_template() {
		...
		<div class="e-n-tabs" data-widget-number="{{ elementUid }}" aria-label="<?php echo esc_html__( 'Tabs. Open items with Enter or Space, close with Escape and navigate using the Arrow keys.', 'ln-elementor' ); ?>">
			<# if ( settings['tabs'] ) { #>
			<div class="e-n-tabs-heading" role="tablist">
